...
//        List<Child> children = childRepository.findChildByParentId(booking.getParentId());
//        if (children.isEmpty()) {
//            throw new ApiException("No children found for parent with ID " + booking.getParentId());
//        }
//
//        // Validate Child IDs
...
//                // Check if babysitter has Special Needs certification
...
//        Boolean babySitterAvailability = isBabySitterAvailable(booking.getBabySitterId(),booking.getStartTime(),booking.getRequestEndTime());
...
//
//        // Log total price before saving to check if it’s correct
//        System.out.println("Total Price Before Saving: " + totalPrice);
//
...
//
//        // Log after save to ensure it's been saved correctly
//        System.out.println("Booking Saved: " + booking.getTotalPrice());
//
//        return totalPrice;
//
//    }
    ...
    public Double createBooking(Booking booking) {
        ...
        if (booking.getDiscountCode() != null && !booking.getDiscountCode().isEmpty()) {
            try {
                ...
                booking = bookingRepository.findBookingById(booking.getId()); // Reload to get updated price
...
//
//        // Calculate total price for recurring bookings or single bookings
//        if (Boolean.TRUE.equals(booking.isRecurring())) {
...
//
//            // Fetch all recurring bookings for this parent, babysitter, and recurrence frequency
...
//            }
//
//            // Loop through each session in the recurring bookings
//            for (Booking session : recurringBookings) {
//                if (!isBabySitterAvailable(session.getBabySitterId(), session.getStartTime(), session.getRequestEndTime())) {
//                    throw new ApiException("Babysitter is unavailable for one or more recurring sessions.");
//                }
//
//                // Accumulate the total price for all sessions
...
////        if (discountCode > 0) {
////            totalPrice -= (totalPrice * (discountCode / 100));
////        }
//
//        // Retrieve the parent and validate balance
//        Parent parent = parentRepository.findParentByid(booking.getParentId());
//        if (parent == null) {
...
//                applyDiscountToBooking(recurringBooking.getId(), booking.getDiscountPercentage());
//            }
//
