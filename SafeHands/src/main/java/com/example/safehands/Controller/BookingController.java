...
public class BookingController {
    ...
    @PostMapping("/add")
    public ResponseEntity createRecurringBooking(@Valid @RequestBody Booking booking, Errors errors) {
       if(errors.hasErrors()){
    ...
    public ResponseEntity updateBooking(@PathVariable Integer id, @Valid @RequestBody Booking updatedBooking,Errors errors) {
        if(errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
