...
public class CourseController {
    ...
    public ResponseEntity addTraining(@PathVariable Integer adminId,@RequestBody @Valid Course course, Errors errors){
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateTraining(@PathVariable Integer courseId,@PathVariable Integer adminId, @RequestBody @Valid Course course,Errors errors){
        if(errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
