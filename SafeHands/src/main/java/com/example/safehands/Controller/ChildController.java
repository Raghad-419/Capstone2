...
public class ChildController {
        ...
        // Create a new child
        @PostMapping("/add")
        public ResponseEntity createChild(@Valid @RequestBody Child child, Errors errors) {
            if (errors.hasErrors()) {
        ...
        public ResponseEntity updateChild(@PathVariable Integer id, @Valid @RequestBody Child updatedChild, Errors errors) {
            if (errors.hasErrors()) {
                return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
            }
            Child child = childService.updateChild(id, updatedChild);
