...
public class ReviewController {
    ...
    // Create a new review
    @PostMapping("/add")
    public ResponseEntity createReview(@Valid @RequestBody Review review , Errors errors) {
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateReview(@PathVariable Integer id, @Valid @RequestBody Review updatedReview,Errors errors) {
        if(errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
