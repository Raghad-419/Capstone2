...
public class ParentController {
    ...
    // Create a new parent
    @PostMapping("/add")
    public ResponseEntity addParent(@RequestBody @Valid Parent parent, Errors errors) {
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateParent(@PathVariable Integer id, @Valid @RequestBody Parent updatedParent,Errors errors) {
        if(errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
